...
static char gps_neo6m_tx_buffer[GPS_NEO6M_TX_BUFFER_SIZE];  //  TX Buffer
static char gps_neo6m_rx_buffer[GPS_NEO6M_RX_BUFFER_SIZE];  //  RX Buffer
static char gps_neo6m_parser_buffer[GPS_NEO6M_PARSER_BUFFER_SIZE];  //  Buffer for ATParser
...
static bool send_raw_command(struct gps_neo6m *dev, const char *cmd) {
...
static bool send_command(struct gps_neo6m *dev, enum CommandId id) {
    assert(dev);
...
static bool send_command_int(struct gps_neo6m *dev, enum CommandId id, int arg) {
...
/// If first time we are opening the driver: Prepare the GPS_NEO6M transceiver for use.  Lock the UART port.
static int gps_neo6m_open(struct os_dev *dev0, uint32_t timeout, void *arg) {
    ...
    internal_init(
        gps_neo6m_tx_buffer, GPS_NEO6M_TX_BUFFER_SIZE,
        gps_neo6m_rx_buffer, GPS_NEO6M_RX_BUFFER_SIZE,
...
int gps_neo6m_config(struct gps_neo6m *drv, struct gps_neo6m_cfg *cfg) {
    ...
    struct gps_neo6m_cfg *drv_cfg = &drv->cfg;
...
/////////////////////////////////////////////////////////
//  GPS_NEO6M Driver Interface

int gps_neo6m_start(void) {
    //  Start the GPS driver. Return 0 if successful.
    //  device_name is the GPS_NEO6M device name e.g. "gps_neo6m_0".
    const char *device_name = GPS_NEO6M_DEVICE;
    assert(device_name);

    //  Init the callout to handle received UART data.
    os_callout_init(&rx_callout, os_eventq_dflt_get(), rx_callback, NULL);

#if MYNEWT_VAL(GPS_NEO6M_ENABLE_PIN) >= 0
    //  Enable GPS module: Set PA1 to high for Ghostyu L476 dev kit
    hal_gpio_init_out(MYNEWT_VAL(GPS_NEO6M_ENABLE_PIN), 0);
#endif  //  GPS_NEO6M_ENABLE_PIN

    {   //  Lock the GPS_NEO6M driver for exclusive use.  Find the GPS_NEO6M device by name.
        struct gps_neo6m *dev = (struct gps_neo6m *) os_dev_open(device_name, OS_TIMEOUT_NEVER, NULL);  //  GPS_NEO6M_DEVICE is "gps_neo6m_0"
        assert(dev != NULL);
        ...
        if (!power_standby_wakeup()) {
            ...
            //  to pass GPS_NEO6M events to this function.
            int rc = gps_neo6m_connect(dev);
            assert(rc == 0);
        }
