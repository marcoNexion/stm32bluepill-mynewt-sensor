...
#include "util.h"
#include "gps_neo6m/gps_neo6m.h"  //  Specific to device

//  Define the device specifics here so the device creation code below can be generic.
#define DEVICE_NAME        GPS_NEO6M_DEVICE  //  Name of device
#define DEVICE_DEV         gps_neo6m         //  Device type
#define DEVICE_INSTANCE    gps_neo6m         //  Device instance
#define DEVICE_CFG         gps_neo6m_cfg     //  Device config
#define DEVICE_CFG_DEFAULT gps_neo6m_default_cfg  //  Device default config
...
void DEVICE_CREATE(void) {
    ...
    if (strlen(MYNEWT_VAL(GPS_DEVICE)) == 0){
        console_printf("%sno instance... " DEVICE_NAME "\n", _gps);
        return;
    }else{
