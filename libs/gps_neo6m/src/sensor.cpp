...
static int gps_neo6m_sensor_get_config(struct sensor *, sensor_type_t, struct sensor_cfg *);

//  Global instance of the sensor driver
static const struct sensor_driver g_gps_neo6m_sensor_driver = {
    gps_neo6m_sensor_read,
    gps_neo6m_sensor_get_config
};
...
int gps_neo6m_sensor_init(struct gps_neo6m *dev, void *arg) {
    ...
    rc = gps_neo6m_default_cfg(&dev->cfg);
    if (rc) { goto err; }
    ...
    rc = sensor_set_driver(sensor, SENSOR_TYPE_GEOLOCATION,
        (struct sensor_driver *) &g_gps_neo6m_sensor_driver);
    if (rc != 0) { goto err; }
...
static int gps_neo6m_sensor_read(struct sensor *sensor, sensor_type_t type,
...
static int gps_neo6m_sensor_get_config(struct sensor *sensor, sensor_type_t type,
 ...
 *
 * @param Sensor device gps_neo6m structure
 * @param Sensor device gps_neo6m_cfg config
 *
